...
fn postgres_parameterizes_where_values_in_textual_order() {
    ...
    plan.r#where = Some(Predicate::And {
        left: Box::new(Predicate::Comparison {
            ...
            right: Box::new(literal(json!(10), DataType::Int)),
        }),
        right: Box::new(Predicate::Like {
            expr: Box::new(column_ref("users", "name")),
            pattern: "A%".to_owned(),
        }),
    });
    ...
    plan.select = vec![
        ...
        Projection::Expr {
            expression: Expression::FunctionCall {
                name: "COUNT".to_owned(),
                args: Box::new(vec![column_ref("a", "id")]),
                distinct: false,
            },
...
fn dialects_use_expected_quoting_placeholders_and_pagination() {
    ...
    plan.r#where = Some(Predicate::Comparison {
        ...
        op: ComparisonOperator::Eq,
        right: Box::new(literal(json!(true), DataType::Boolean)),
    });
    plan.limit = Some(10);
    plan.offset = Some(20);
...
fn cte_parameters_share_one_postgres_placeholder_sequence() {
    ...
    plan.r#where = Some(Predicate::Comparison {
        left: Box::new(column_ref("active_users", "id")),
        op: ComparisonOperator::Gt,
        right: Box::new(literal(json!(100), DataType::Int)),
    });
...
fn postgres_cte_literals_get_cast() {
    let cte_query = QueryPlan {
        ...
        r#where: Some(Predicate::Comparison {
            left: Box::new(column_ref("users", "level")),
...
fn literal_content_is_never_interpolated_into_sql() {
    ...
    let mut plan = base_plan();
    plan.r#where = Some(Predicate::Comparison {
        left: Box::new(column_ref("users", "name")),
        op: ComparisonOperator::Eq,
        right: Box::new(literal(json!(malicious), DataType::String)),
    });
...
fn unsafe_function_name_is_rejected_instead_of_interpolated() {
    ...
    plan.select = vec![Projection::Expr {
        expression: Expression::FunctionCall {
            ...
            args: Box::new(vec![column_ref("users", "id")]),
            distinct: false,
        },
        alias: None,
    }];
...
fn in_between_not_and_is_null_are_rendered_and_parameterized() {
    ...
    plan.r#where = Some(Predicate::And {
        left: Box::new(Predicate::Between {
            ...
            low: Box::new(literal(json!(1), DataType::Int)),
            high: Box::new(literal(json!(10), DataType::Int)),
        }),
...
fn query_builder_postgres_placeholders_renumber_per_invocation() {
    ...
    plan.r#where = Some(Predicate::Comparison {
        left: Box::new(column_ref("users", "name")),
        op: ComparisonOperator::Eq,
        right: Box::new(literal(json!("alice"), DataType::String)),
    });
    let validated = validated(plan);
...
fn postgres_compiles_union_all() {
    let right = QueryPlan {
        ...
        r#where: Some(Predicate::Comparison {
            left: Box::new(column_ref("users", "id")),
            op: ComparisonOperator::Gt,
            right: Box::new(literal(json!(50), DataType::Int)),
        }),
        group_by: None,
        having: None,
        order_by: None,
...
fn postgres_compiles_decimal_literal() {
    let mut plan = base_plan();
    plan.r#where = Some(Predicate::Comparison {
        left: Box::new(column_ref("users", "price")),
        op: ComparisonOperator::Gt,
        right: Box::new(literal(json!(99.99), DataType::Decimal)),
    });
    let compiled = PostgresCompiler
        .compile(&validated(plan))
