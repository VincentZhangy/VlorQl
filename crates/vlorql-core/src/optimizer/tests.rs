...
use crate::schema::{
    ...
    ForeignKey, FromClause, JoinClause, JoinType, Predicate, Projection, QueryPlan, SchemaMetadata,
...
fn pipeline_applies_rules_in_order() {
    ...
    assert_eq!(conjunct_count(cte_where), 2);
    assert!(
        super::analyze::split_conjuncts(cte_where)
            .iter()
            .any(|p| { matches!(p, Predicate::Comparison { right, .. } if **right == int(100)) })
    );
...
fn column_pruning_preserves_primary_and_foreign_keys() {
    ...
    let cte_cols: Vec<&str> = rewritten.ctes.as_ref().unwrap()[0]
        .query
        .select
        .iter()
        .filter_map(|p| match p {
