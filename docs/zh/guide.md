...
| 规则 | 效果 |
|------|------|
| 常量折叠 | 计算常量子表达式（`100 + 50` → `150`）并简化代数恒等式（`x + 0` → `x`、`true AND x` → `x`）。 |
| 谓词下推 | 将 `WHERE` 合取项移动到 CTE 体内部，以便尽早过滤。支持**多层级联**穿透嵌套 CTE。 |
| 列剪枝 | 移除 CTE 输出中未被引用的列。聚合函数的参数列仅在该聚合结果被引用时才保留。 |
| 连接重排序 | 重新排序 `INNER JOIN` 链以最小化总成本（需要统计信息）。使用位图加速的 DP 搜索。 |

管道可选择在**固定点迭代**模式下运行（最多 3 轮），以捕获级联效应——常量折叠可能产生新的下推机会，下推后又可能让更多列可被剪枝。直接使用 `QueryOptimizer::optimize_repeat()` 或 `RewriterPipeline::repeat_until_stable()`。
